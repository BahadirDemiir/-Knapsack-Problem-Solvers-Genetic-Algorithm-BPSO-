import random
class Bag:
    esyalar = ["telefon", "yüzük", "sürahi", "tablet", "laptop", "trabzon burması", "fenerbahçe forması imzalı", "ütü", "tost makinesi", "ansiklopedi"]
    ...
    def caprazlama(populasyon):
        cantalarıSeçmeÜstSınır = len(populasyon)
        olasılık = random.randint(0, 100)
        sayac = len(populasyon)
        while sayac < 100:
            deger1 = random.randint(0, cantalarıSeçmeÜstSınır - 1)
            deger2 = random.randint(0, cantalarıSeçmeÜstSınır - 1)
            child = Bag()
            child.cantaninIci = [populasyon[deger1].cantaninIci[i] if random.choice([True, False]) else populasyon[deger2].cantaninIci[i] for i in range(10)]
            if olasılık > 85:
                Bag.mutasyon(child)
